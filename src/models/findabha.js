...
const searchabha = async (accessToken, mobile) => {
  try {
    ...
    const response = await axios.post(
      `${config.abdm.abhaBaseUrl}/api/v3/profile/account/abha/search`,
      { scope:["search-abha"],
        mobile: mobile,
      },
      {
...
const sendindexotp = async ({ accessToken, txnId, indexid }) => {
  try {
    
    if (!accessToken || !txnId || !indexid) {
      throw new Error('Missing required parameters: accessToken, txnId, indexid');
    }
...
const verifyindexOtp = async ({ accessToken, txnId, otp }) => {
  try {
